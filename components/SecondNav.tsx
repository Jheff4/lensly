    ...
    <nav className='flex-center bg-mirage-100 fixed top-0 z-20 w-full py-6 text-white'>
      <div className={`${!navbar ? "flex justify-between max-md:flex-between max-md:flex-wrap mx-auto w-full max-w-screen-2xl lg:px-20 xs:px-8 sm:px-10 max-xs:px-3" : "flex-between mx-auto w-full transition-all max-w-screen-2xl lg:px-10 xs:px-8 sm:px-10 max-xs:px-3"}`}>
        ...
        {navbar ? (<ul className='flex-center gap-x-3 md:gap-x-3 xl:gap-x-7 max-md:hidden md:text-[0.9rem] lg:text-lg'>
          <li id='dashboard' className='!font-bold'>
            <Link
              href="/dashboard"
              target=""
            >
        ...
        <div className='flex flex-row gap-[16px]'>  
          ...
          /> ) : ( <HiX className="block md:hidden bg-ebonyClay border border-solid border-[#74738B] p-1 font-semibold rounded-[8px] transition-all text-[40px] order-2 cursor-pointer"
            onClick={() => setNavbar(!navbar)}
          />) }
        </div>
